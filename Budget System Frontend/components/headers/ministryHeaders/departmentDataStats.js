...
export default function DepartmentDataStats() {
    ...
    const { ministryId, departmentId } = router.query; // Get both ministryId and departmentId from query params
    ...
    async function getRequestsInDepartment(){
        await BudgetRequestServices.getBudgetRequestInDepartment(departmentId)
            .then((res) => {
                const { data } = res;
                setRequests(data);
            })
