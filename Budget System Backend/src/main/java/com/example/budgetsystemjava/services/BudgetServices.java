...
public class BudgetServices {
    ...
    public List<BudgetDTO> getPendingRequests() {
        List<Budget> pendingRequests = budgetRepo.findByStatus("created");

        return getBudgetDTOS(pendingRequests);
    }

    public void saveStatus(BudgetDTO budgetDTO) {
        Budget budget = modelMapper.map(budgetDTO, Budget.class);
        Budget savedBudget = budgetRepo.save(budget);
        modelMapper.map(savedBudget, BudgetDTO.class);
    }

    public BudgetDTO getById(Long requestId) {
        Budget budget = budgetRepo.findById(requestId)
                .orElse(null);
        return modelMapper.map(budget, BudgetDTO.class);
    }


    public List<BudgetDTO> getHandledRequests() {
